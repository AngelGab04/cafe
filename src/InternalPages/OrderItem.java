...
import MainPackage.OrderItemFrame;
 ...
 * @author Rhai
 */
public class OrderItem extends javax.swing.JInternalFrame {
    ...
    public void displayData(){
        try{
            ...
            ResultSet rs = dbc.getData("SELECT order_item_id, order_id, product_id, quantity, unit_price FROM tbl_order_item");
            orderitem_tbl.setModel(DbUtils.resultSetToTableModel(rs));
            DefaultTableModel model = (DefaultTableModel) orderitem_tbl.getModel();
            String[] columnIdentifiers = {"Order Item ID", "Order ID", "Product ID", "Quantity", "Unit Price"};
    ...
    private void initComponents() {
        ...
        orderitem_tbl = new javax.swing.JTable();
        ...
        jScrollPane1.setViewportView(orderitem_tbl);
    ...
    private void add_btnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_add_btnMouseClicked
        ...
        mainFrame.dispose();
        OrderItemFrame order = new OrderItemFrame();
    ...
    private void edit_btnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_edit_btnMouseClicked
        int rowIndex = orderitem_tbl.getSelectedRow();
        if(rowIndex < 0){
            JOptionPane.showMessageDialog(null, "Please Select an Item!");
        }else{
            TableModel model = orderitem_tbl.getModel();
            
            OrderItemFrame orderitem = new OrderItemFrame();
            orderitem.orderitemid_inp.setText(""+model.getValueAt(rowIndex, 0));
            String oid = model.getValueAt(rowIndex, 0).toString();
            try{
            DBconnector dbc = new DBconnector();
            ResultSet rs = dbc.getData("SELECT * FROM tbl_order_item WHERE order_item_id = '"+oid+"' ");
            if(rs.next()){
            orderitem.orderidf_inp.setText(rs.getString("order_id"));
            orderitem.prodidf_inp.setText(rs.getString("product_id"));
            orderitem.orderitemquantity_inp.setText(rs.getString("quantity"));
            orderitem.orderitemunit_inp.setText(rs.getString("unit_price"));
            
            orderitem.setVisible(true);
            orderitem.action = "Update";
            orderitem.action_lbl.setText("UPDATE");
            JFrame mainFrame = (JFrame) SwingUtilities.getWindowAncestor(this);
            mainFrame.dispose();
            }else{
    ...
    private void delete_btnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_delete_btnMouseClicked
        ...
        if(rowIndex < 0){
            ...
            if(a == JOptionPane.YES_OPTION){
                DBconnector dbc = new DBconnector();
                int orderitem_id = Integer.parseInt(id);
                dbc.deleteData(orderitem_id, "tbl_order_item", "order_item_id");
                displayData();
            }
        }
    ...
    private void print_btnMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_print_btnMouseClicked
        ...
        try{
            orderitem_tbl.print(JTable.PrintMode.FIT_WIDTH, header, footer);          
    ...
    private javax.swing.JTable orderitem_tbl;
